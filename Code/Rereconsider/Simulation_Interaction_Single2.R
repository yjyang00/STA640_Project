...
result = foreach (i = 1:S, .combine = 'rbind', .errorhandling='remove') %dopar% {
  ...
  dat = tidyr::expand_grid(data.frame(id = 1:n, A = A, D = D), Time = 1:t) %>% 
    ...
    mutate(Y = (Time)^2 + gamma*A + delta*X_it + rho*D_it + phi*(A)^3*D_it + epsilon)
...
result_10_5_gamma3_phi0 = result
save(result_10_5_gamma3_phi0, file = "result_data/result_10_5_gamma3_phi0.RData")
