...
bias_mutate <- function(df) {
  df %>%  mutate(result = panel_bias_sim(S = 1000, n, t, t_treat, delta, gamma, rho, confound_treatment))
}

bias_result_full <- tidyr::expand_grid(n, t, t_treat, delta, gamma, rho, confound_treatment) %>% 
...
# save(bias_result_full, file = "bias_result_full.RData")