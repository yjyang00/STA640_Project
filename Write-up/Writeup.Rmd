...
Figure \ref{fig:all_bias_with_ols} shows average bias of estimated treatment effects from Did, FE, OLS, and RE estimation methods under three degrees to which A affects D with N = 100, t = 10 and t-treat = 5. With no interaction and no time-varying unmeasured confounders, DiD and FE model give unbiased estimate, and OLS model has the largest bias. As $\gamma$ increases, OLS bias keeps increasing in a linear trend while FE bias fluctuates at a small level. As $\rho$ increases, bias doesn't change with $\rho$.

Figure \ref{fig:bias_with_t} shows distribution of bias across varying total time points. With no interaction and no time-varying unmeasured confounders, FE model gives unbiased estimate, RE model has higher bias, and OLS has the largest bias. As $T$, the number of time points increases, RE bias decreases, which may result from increasing number of data points.
...
In Figure \ref{fig:bias_interaction}, we show that even fixed effects models are not able to unbiasedly estimate the treatment effects when there are interaction terms between the unmeasured confounding and treatment assignment or time. Specifically, Figure \ref{fig:bias_AD_interaction} and \ref{fig:bias_AT_interaction} both show that the bias increases as $\phi$ and $\beta$ increases. The reason why we see 
