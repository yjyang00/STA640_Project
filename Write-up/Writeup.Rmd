...
First of all, Figure \ref{fig:all_bias_with_ols} describes the average bias of estimated treatment effects from DID, FE, OLS, and RE estimation methods under three degrees to which A affects D with `N` = 100, `T` = 10 and `T-treat` = 5. 
...
The deviation of the FE bias from 0 is reasonable since fixed effects can no longer absorb the unmeasured confounders under large $\phi$. In addition, a similar pattern is displayed in \ref{fig:bias_AT_interaction}, where the three types of bias increase as $\beta$ increases under fixed time points $T$. 
This is because large $\beta$ makes it more challenging for fixed effects to absorb unmeasured confounders. 

Bias-corrected random effects models: Figure \ref{fig:bias_correct_RE} and Figure \ref{fig:bias_BiasCorrect_inter}
...
Binary Case (outcome, confounding)?

More sophiscated time-varying treatment?

More complicated interaction beween confounding and treatment?
