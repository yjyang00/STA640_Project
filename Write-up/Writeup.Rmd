...
In addition, we also show in Figure \ref{fig:CI_interaction} that the coverage of 95% CI of treatment effect estimates drops sharply as we allow the unmeasured confounding to be time-varying and interacts with the treatment, implying that the estimators are far from being consistent under these setttings. 
...
bias_result_interaction_gamma %>% pivot_longer(cols = c("DID_bias","OLS_bias","FE_bias", "RE_bias"), names_to = "Model", values_to = "Bias") %>%
  filter(gamma < 3) %>% ggplot(aes(x = gamma, y = Bias, color = Model)) + geom_point() + geom_line() + facet_wrap(~confound_treatment) + labs(x = expression(gamma))
,
