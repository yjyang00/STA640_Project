...
In the case of continous outcomes, our data generation process (DGP) uses the following model specifications:
...
In our most basic DGP of artificial panel data, there are 100 individuals and 10 time points. Covariate, $X$, for each individual is simulated from $N(0,1)$. The unmeasured counfounding, $A$, is either from $N(0,1)$ or $\text{Uniform(0,1)}$. At half time ($t=5$), treatment is given to those in the treatment group. 
...
Second, $A\sim \text{Uniform}(0,1)$ directly serves as the probability of receiving the treatment. We regard this as `very strong` influnce of A on D.  
...
* The true coefficient of the interaction between the treatment and the unmeasured confounding, $\gamma$, could var from to 0 to 10, with zero meaning that there is no interaction effect. 
...
The basic form of OLS model is very likely to be vulnerable to severe bias since it does not remedy unmeasured confounding or potential interactions. 
...
The formulation and validity of the fixed effects model are discussed extensively in the literature. Despite that, whether the individual intercepts can fully absorb all types of unmeasured confoundings remains questionable.   
...
***Random effects (RE/RI) model***

The random effects model is essentially a random intercept (RI) model. The argument that RI could account for unmeasured confounding is theoretically problematic due to its modeling specification. 
...
We use `lmer` from the `lme4` package to fit the random intercept model using REML. This frequentiest way of fitting random intercept models is often used, though it may have as robust uncertainty quantifications as the Bayesian hierarhical models.  
...
Figure \ref{fig:bias_interaction}
