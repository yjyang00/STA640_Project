...
As such, our projects seeks to unravel the trends of potential bias, if it exists, when applying fixed/random effects models in the events of unmeasured confounding.
...
We use `lm` to fit OLS and fixed effects models and `lmer` from the `lme4` package to fit the random intercept model using REML. The frequentiest way of fitting random intercept models is commonly employed and faster to compute, though it may not have as robust uncertainty quantification as the Bayesian hierarchical models.  
...
For each of the setting and estimator described above, we compute an average estimate, bias and 95% confidence interval of the treatment effect based on 1000 simulations of data generation and model fitting. There are some interesting results elaborated below. 
...
In addition, a similar pattern is displayed in Figure \ref{fig:bias_AT_interaction}, where the three types of bias increase as $\beta$ increases under fixed time points $T$. 
This is because large $\beta$ makes it more challenging for fixed effects to absorb unmeasured confounders. 
...
This paper does not provide the closed form of the asymptotic bias. [@li2020impact] derived the closed form of bias in the presence of unmeasured within- and/or between-cluster confounders when the treatment is continuous. Particularly, the formula suggests a positive linear relationship between the coefficient of unmeasured confounder and the bias. However, under a different assumption that the treatment is binary, this paper presents a slightly different pattern that the RE bias first increases and then decreases as coefficient of unmeasured confounder $\gamma$ grows larger. Future work can be done regarding the closed form of bias under binary treatment and will be helpful for obtaining insights of asymptotic behavior of the bias.

It is possible to further explore concerning the following aspects. Firstly, fixed/random effects model does not estimate the strength of unmeasured confounders as latent variable on the outcome variable. Other models including Instrumental Variable (IV) may be able to address the problem.
Secondly, we have tried binary outcome which presents a similar behavior to the simulation. It is possible to conduct simulation for binary confounders because the real-word latent variable is likely to be binary.
...
Fourthly, the coefficient of treatment in this paper is set to be constant. Future work can specify more complicated treatment effect which may change with time. 
Finally, three types of interactions between confounders and treatment is considered in this paper, more types can be explored in future research.  
